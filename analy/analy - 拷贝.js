		...
		var $i = (p.items)?$a.querySelectorAll(p.items):[$a];
			...
			for(var i=0;i<r.total;i++){
				...
				for(var x in p.d){
					...
					// 获取对象
					if(!pdx.d||pdx.d==1||pdx.d==-1){	// 常规
						$d = pdx.dom?(pdx.d?$t.querySelectorAll(pdx.dom):[$t.querySelector(pdx.dom)]):[$t];
					
						dl = $d.length;
						updxd=pdx.d;
						updxget=pdx.get;
						if(!pdx.d&&!$d[0]){$d=[];dl=0};	// 单值未获
					}
					
					if(!dl)continue;
					
					// 过滤子集
					if(pdx.filte){for(var y=0;y<dl;y++){$d[y] = _.Dr($d[y],pdx.filte)}};	// 过滤子集
					
					// 获取基础值
					if(pdx.get == 4){	// dom数量
						td = [$d.length];
					}else if(pdx.d != 3){// 常规获值
						td = [];for(var y = 0;y<dl;y++){td.push(_valDom($d[y],pdx))};
					}else{	// 引值(值都为数组)
						td = (_.typeOf(td) == '[object Array]')?td.concat():[td];	// 复制数组,避免通值 字符串|数值|数组
					}
					
					
					// 值运算
					if(pdx.arr==1||(!pdx.arr&&(!pdx.d||pdx.d==-1||(pdx.d>=2&&(!updxd||updxd==-1))||pdx.get==4||updxget==4))){// 合并值(单值)
						td = (dl==1||pdx.get==4)?td[0]:td.join(pdx.fg||'');
						td = _valPro(td,pdx);
					}else{
