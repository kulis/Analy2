...
innerXHTML=function(source){if(typeof(source)=='string')source=document.getElementById(source);if(!(source.nodeType==1))return false;var _children=source.childNodes;var _html='';for(var c=0;c<_children.length;c++){if(_children[c].nodeType==3){var $text_content=_children[c].nodeValue;$text_content=$text_content.replace(/</g,'<');$text_content=$text_content.replace(/>/g,'>');_html+=$text_content}else if(_children[c].nodeType==1&&_children[c].nodeName.substr(0,1)=="/"){}else if(_children[c].nodeType==8){}else{_html+='<'+_children[c].nodeName.toLowerCase();var $attributes=_children[c].attributes;for(var a=0;a<$attributes.length;a++){var $attName=$attributes[a].nodeName.toLowerCase();var $attValue=$attributes[a].nodeValue;if($attName=='style'&&_children[c].style.cssText){_html+=' style="'+_children[c].style.cssText.toLowerCase()+'"'}else if($attValue&&$attName!='contenteditable'){_html+=' '+$attName+'="'+$attValue+'"'}}_html+='>'+innerXHTML(_children[c]);_html+='</'+_children[c].nodeName.toLowerCase()+'>'}}return _html}
